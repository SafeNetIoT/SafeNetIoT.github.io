...
export const Projects = ({ title, cards }) => {
	return (
		<div id="Research" className="bg-primary py-5 px-5">
			<div className="container">
				<h1 className="text-light fw-bold">Research</h1>
				<div className="d-flex flex-row flex-wrap justify-content-center">
					{cards.map((value, index) => (
						<Card
							key={index}
...
export const Card = ({ title, description, content, icons }) => {
	return (
		<div className="card py-3 px-3 mx-sm-4 my-4 card-work" style={{ width: "20rem" }}>
			...
			{content && (
				<div className="card-content">
					<p style={{ color: 'lightgray', fontStyle: 'italic' }}>{content}</p>
				</div>
			)}
				...
				{icons && icons.map((value, index) => (
					<Link key={index} href={value.link}>
						<a target="_blank" rel="noreferrer">
							<FontAwesomeIcon
								className="icon-style mx-1" icon={value.icon} size="2x" />
						</a>
					</Link>
				))}
			</div>
